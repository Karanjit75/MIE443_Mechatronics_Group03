...
static std::vector<WristViewPose> buildWristObservationPoses(double obj_x, double obj_y, double obj_z)
{
    // SAFE OPTION:
    // Keep a single fixed camera orientation and move around the approximate object location.
    // EDIT THESE if needed after real testing.
...
{
    // SAFE OPTION: conservative travel pose from earlier testing
...
int main(int argc, char** argv)
{
    ...
    // Original robot pose - must return here at the end
    ...
    // Do NOT use RViz joint sliders here. Put the actual object Cartesian position.
    // -------------------------------------------------------------------------
    ...
    for (size_t i = 0; i < boxes.coords.size(); ++i) {
        ...
        //   cup coordinate   -> edit corresponding coords.xml x,y,phi
        // ---------------------------------------------------------------------
        obs.x = boxes.coords[i][0];
        obs.y = boxes.coords[i][1];
        obs.phi = boxes.coords[i][2];
            ...
            if (!scenesVisited) {
                ...
                for (size_t i = 0; i < scene_observations.size() && rclcpp::ok(); ++i) {
                    ...
                    if (detected_ok) {
                        ...
                        // Drop when cup scene is found, but continue visiting the rest afterward
                        if (!cupDropped && isCupMatch(obs.normalized_name, obs.confidence, 0.50f)) {
                            RCLCPP_INFO(
                                node->get_logger(),
                                "Cup detected at scene %d with confidence %.2f. Dropping now.",
                                obs.index, obs.confidence
                            );

                            bool dropped = dropObjectToBin(node, armController);
                            if (dropped) {
                                cupDropped = true;
                                RCLCPP_INFO(node->get_logger(), "Cup dropped at scene %d", obs.index);
                            } else {
                                RCLCPP_WARN(node->get_logger(), "Drop attempt failed at scene %d", obs.index);
                            }

                            bool travel_ok = moveArmToTravelPose(armController);
                            if (!travel_ok) {
                                RCLCPP_WARN(node->get_logger(), "Could not move arm to travel pose after drop.");
                            }
                        }
                    } else {
