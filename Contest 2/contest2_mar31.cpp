    ...
    std::string& detected_item, float& confidence)
{
    ...
    return !detected_item.empty() && valid_objects.count(detected_item) && confidence > 0.05f;
}
...
int main(int argc, char** argv) {
    ...
    float manipulable_confidence = 0.42f;
    ...
    // Execute strategy
    while (rclcpp::ok() && secondsElapsed <= 9999) {
        rclcpp::spin_some(node);
        ...
        switch (current_state) {
            ...
            case RobotState::SCAN_MANIPULABLE_OBJECT: {
                ...
                if (!detected.empty() && valid_objects.count(detected) && conf > 0.05f) {
            ...
            case RobotState::DETECT_OBJECT: {
                ...
                if (outfile.is_open()) {
                    outfile << detected_item
                        << " | confidence=" << confidence
                        << " | coordinate_index=" << current_target_index
                        << "\n";
                }
                break;
            }
            ...
            case RobotState::ROTATE_SLIGHTLY: {
                std::vector<double> rotate_offsets = {0.0, -M_PI/6, M_PI/6, -M_PI/3, M_PI/3};
                ...
                if (reached_goal) {
                    ...
                    if (!detected_item.empty() && valid_objects.count(detected_item) && confidence > 0.05f) {
