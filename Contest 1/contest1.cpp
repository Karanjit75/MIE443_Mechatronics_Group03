...
#include <ctime>        // initialize seed for randomization in turn direction
...
private:
    ...
    void controlLoop()
    {
        ...
        const float Stop_Dist = 0.4;  // If obstacle closer than 0.3 m, turn
        const float Slow_Dist = 0.7;  // If near obstacle zone, speed <= 0.10 m/s
        const float Fast_Speed = 0.25; // <=0.25 m/s maximum
        const float Slow_Speed = 0.10; // slow-zone speed cap (<=0.10 m/s)
        const float Turn_Speed = M_PI / 3; // Turning speed (in rad/s)
        ...
        if (any_bumper_pressed && !recovering) {
            ...
            if (LeftPressed) {
                linear_ = -2.0 // Reverse a bit first
                ...
                if (minLaserDist_ < Stop_Dist) {
                    // Too close then rotate away to more open side
                    linear_ = 0.0;
                    if (left_min_dist_ > right_min_dist_) {
                        angular_ = Turn_Speed;  // left side has more open space
                    }
                    else if (right_min_dist_ > left_min_dist_) {
                        angular_ = -Turn_Speed; // right side has more open space
                    }
                    else {
                        // if both same distance then random turn (either left/right)
                        if ((rand()%10 +1) > 7) {
                            angular_ = Turn_Speed;  // left turn
                        }
                        else {
                            angular_ = -Turn_Speed; // right turn
                        } 
                    }              
            }
            else {
...
int main(int argc, char** argv)
{
    ...
    srand(time(nullptr)); // added so that random choice changes every run
