        ...
        switch (state) {
            ...
            case ContestState::Visit_Scene_Objects:
            {
                ...
                for (int view_try = 0; view_try < max_scene_view_attempts; ++view_try) {
                    ...
                    if (view_try > 0) {
                        RCLCPP_INFO(node->get_logger(), "Repositioning for better view at box %zu (try %d/%d)", current_box_index, view_try + 1, max_scene_view_attempts);
                    ...
                    std::string detected_object =
                        yoloDetector.getObjectName(CameraSource::OAKD, false);
                ...
                if (!best_detected_object.empty()) {
                    scene_detections.push_back({
                        ...
                        best_conf,
                        goal_x,     // Store x coordinate
                        goal_y,     // Store y coordinate
                        goal_phi    // Store orientation
                    });
                    ...
                    if (!matching_object_found &&
                        best_detected_object == manipulable_object_name)
                    {
