...
private:
    ...
    float choose_open_space(float left_d, float right_d) {
        ...
        else if (right_d > left_d + 0.001) {
            return -1.0;
        }
        else {
            // If almost equal, use frontier memory

            // Project a point ahead to the left and right
            left_sample_x = pos_x_ + std::cos(yaw_ + M_PI/4);
            left_sample_y = pos_y_ + std::sin(yaw_ + M_PI/4);
            right_sample_x = pos_x_ + std::cos(yaw_ - M_PI/4);
            right_sample_y = pos_y_ + std::sin(yaw_ - M_PI/4);

            // Check if those future points are in "visited" territory
            left_visited = isVisited(left_sample_x, left_sample_y);
            right_visited = isVisited(right_sample_x, right_sample_y);

            if (left_visited && !right_visited) {
                return -1.0; // Steer toward the unvisited right
            } else if (!left_visited && right_visited) {
                return 1.0;  // Steer toward the unvisited left
            }
            else {
                //Tie, both visited or both unvisited -> choose alternate direction
                flip_direction *= -1.0;
    ...
    void odomCallback(const nav_msgs::msg::Odometry::SharedPtr odom)
    {
        ...
        // Offset by ARENA_SIZE/2 to handle negative coordinates (assuming 0,0 is center)
        cur_x = static_cast<int>((pos_x_ + (ARENA_SIZE / 2.0)) / CELL_SIZE);
        cur_y = static_cast<int>((pos_y_ + (ARENA_SIZE / 2.0)) / CELL_SIZE);

        // Safety check and mark as visited
        if (cur_x >= 0 && cur_x < GRID_DIM && cur_y >= 0 && cur_y < GRID_DIM) {
            visited_grid_[cur_x][cur_y] = true;
        }
    }
    ...
    void controlLoop()
    {
        ...
        // print grid every 10 seconds for debugging and visualization of frontier exploration
        static rclcpp::Time last_print_time = now;
        if ((now - last_print_time).seconds() > 10.0) {
            printGrid();
            last_print_time = now;
        }
        ...
        const float Turn_Speed = M_PI / 6; // Turning speed (in rad/s)
...
/*
        ...
        if (recovering) {
            ...
            else if (rec_stage == 1 && recovering) {
            ...
            else if (rec_stage == 2 && recovering) {
            ...
            else if (rec_stage == 3 && recovering) {
                double d2 = std::hypot(pos_x_ - rec_start_x, pos_y_ - rec_start_y);
                if (d2 < arc_dist) {
                    linear_ = 0.08;
                    angular_ = 0.5 * recoveryTurnDirection;
                }
                else {
                    recovering = false;
                    rec_stage = 0;
            ...
            if (too_close) {
                ...
                // Calculating base steering directions
                ...
                // (7.2) Frontier-Aware Steering
                // Decide priority: prioritize wall avoidance if one side is significantly closer
                if (std::abs(left_min_dist_ - right_min_dist_) > 0.20) {
                    angular_ = 0.35 * steer_dir;
                }
                else {
                    float curiosity_steer = 0.0;
                    // Project a point ahead to the left and right
                    left_sample_x = pos_x_ + std::cos(yaw_ + M_PI/4);
                    left_sample_y = pos_y_ + std::sin(yaw_ + M_PI/4);
                    right_sample_x = pos_x_ + std::cos(yaw_ - M_PI/4);
                    ...
                    if (left_visited && !right_visited) {
                        curiosity_steer = -0.35; // Steer toward the unvisited right
                    } else if (!left_visited && right_visited) {
                        curiosity_steer = 0.35;  // Steer toward the unvisited left
                    }
                    else {
                        //Tie, both visited or both unvisited -> choose more open side
                        curiosity_steer = 0.35 * choose_open_space(left_min_dist_, right_min_dist_); 
                    }

                    angular_ = curiosity_steer;
                    // Combine with wall avoidance (Wall avoidance should still have priority)
                    //angular_ = (0.35 * steer_dir) + (0.20 * curiosity_steer);
                }
            }
        }   
    ...
    double pos_y_;
    double yaw_;
    int cur_x;      // for grid memory (frontier exploration)
    int cur_y;      // for grid memory (frontier exploration)
    ...
    // Recovery
    int rec_stage = 0;
    rclcpp::Time rec_start_time;
    double rec_start_x = 0.0;
    double rec_start_y = 0.0;
    double rec_target_yaw = 0.0;


    // Define grip memory for frontier exploration
    // Define the arena size and grid resolution
    const double ARENA_SIZE = 5; // Slightly larger than 4.87m for safety
    const double CELL_SIZE = 0.5;  // 50cm cells
    const int GRID_DIM = static_cast<int>(ARENA_SIZE / CELL_SIZE);

    // A 2D grid to track visited cells (initialized to false)
    std::vector<std::vector<bool>> visited_grid_ = 
    std::vector<std::vector<bool>>(GRID_DIM, std::vector<bool>(GRID_DIM, false));

    // Define variables for frontier steering (to encourage exploration of unvisited areas)

    double left_sample_x;
    double left_sample_y;
    double right_sample_x;
    double right_sample_y;
    bool left_visited;
    bool right_visited;

    bool isVisited(double x, double y) {
        // Offset coordinates: assuming (0,0) is the center of the arena
        // We add ARENA_SIZE / 2 to shift the range from [-2.5, 2.5] to [0, 5.0]
        int grid_x = static_cast<int>((x + (ARENA_SIZE / 2.0)) / CELL_SIZE);
        int grid_y = static_cast<int>((y + (ARENA_SIZE / 2.0)) / CELL_SIZE);

        // Safety check: ensure coordinates are within the grid bounds
        if (grid_x >= 0 && grid_x < GRID_DIM && grid_y >= 0 && grid_y < GRID_DIM) {
            return visited_grid_[grid_x][grid_y];
        }
        
        // If out of bounds, treat as "visited" so the robot doesn't try to drive 
    ...
    void printGrid() {
        ...
        for (int y = GRID_DIM - 1; y >= 0; y--) { 
            for (int x = 0; x < GRID_DIM; x++) {
                grid_str += visited_grid_[x][y] ? "[X]" : "[ ]";
            }
            grid_str += "\n";
        }
        grid_str += "----------------------------\n";
        RCLCPP_INFO(this->get_logger(), "%s", grid_str.c_str());
}
