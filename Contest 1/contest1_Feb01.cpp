...
private:
    // Fix speed caps (helper function created):
    ...
    // hazardCallback (/hazard_detection):
    ...
    void controlLoop()
    {
        ...
        Control Loop Strategy:
            ...
            4) If near obstacles -> speed capped at 0.1 m/s
            5) Speed cap always to 0.25 m/s */
       
        // Don't move until odom has arrived at least once (for safety)
        if (!have_odom_) {
            linear_ = 0.0;
            angular_ = 0.0;
        ...
        if (any_bumper_pressed && !recovering) {
            ...
            if (LeftPressed) {
                ...
                recoveryTurnDirection = 1.0;     // If Center bumper hit -> turn left by default
            }
        ...
        if (recovering) {
            ...
            if (minLaserDist_ < Stop_Dist) {
                ...
                linear_ = 0.0;
                angular_ = Turn_Speed; // set to always turn left when blocked (can randomize this later)
            }
            else {
